...
function JobBlock({ job, compact }: { job: CalendarJobItem; compact?: boolean }) {
...
function Cell({ technicianId, date, jobs }: { technicianId: string; date: Date; jobs: CalendarJobItem[] }) {
              ...
              {technicians.map((tech) => (
                <div key={tech.id} className="contents">
                  ...
                  {days.map((d) => {
                    ...
                    return <Cell key={`${tech.id}-${dateKey(d)}`} technicianId={tech.id} date={d} jobs={dayJobs} />;
                  })}
                </div>
              ))}
            </div>
