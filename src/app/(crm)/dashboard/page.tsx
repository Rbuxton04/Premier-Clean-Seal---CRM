...
async function loadKpis(): Promise<{ kpis: Kpi[]; dbOnline: boolean }> {
  ...
  try {
    const [
      ...
      db.job.count({ where: { deletedAt: null, customer: { deletedAt: null }, scheduledStart: { gte: startOfDay } , status: { in: ["BOOKED", "IN_PROGRESS"] } } }),
      db.job.count({ where: { deletedAt: null, customer: { deletedAt: null }, scheduledStart: { gte: startOfWeek }, status: { in: ["BOOKED", "IN_PROGRESS"] } } }),
      db.enquiry.count({ where: { stage: "NEW" } }),
      db.quote.count({ where: { deletedAt: null, customer: { deletedAt: null }, status: { in: ["SENT", "VIEWED"] } } }),
      db.job.count({ where: { deletedAt: null, customer: { deletedAt: null }, status: "IN_PROGRESS" } }),
      db.job.count({ where: { deletedAt: null, customer: { deletedAt: null }, status: "COMPLETED" } }),
